...
ddos_services = {
  ddos_plan_eastus2 = {
    name = "con-{{configs.environment}}-hub-ntw-00-00-a-ddos-00-e2"
  ...
  ddos_plan_centralus = {
    name = "con-{{configs.environment}}-hub-ntw-00-00-a-ddos-00-c1"
...
vnets = {
  ...
  eus2_vnet_01 = {
    ...
    vnet = {
      name          = "con-{{configs.environment}}-hub-ntw-00-00-a-vnet-00-e2"
      address_space = ["{{configs.connectivity_eus2_vnet_01_cidr}}"]      
    }
    specialsubnets = {
      AzureFirewallSubnet = {           # /26 or larger
        name = "AzureFirewallSubnet" #Must be called AzureFirewallSubnet
        cidr = ["{{configs.connectivity_eus2_vnet_01_azurefirewallsubnet_cidr}}"]
      ...
      GatewaySubnet = {         # /29 or larger, /26 or larger recommended
        ...
        cidr = ["{{configs.connectivity_eus2_vnet_01_gatewaysubnet_cidr}}"]
      }      
    }
    ...
    subnets = {
      AzureBastionSubnet = {            # /27 or larger
        ...
        cidr    = ["{{configs.connectivity_eus2_vnet_01_azurebastionsubnet_cidr}}"]
        nsg_key = "eus2_azure_bastion_nsg"
      }    

      
      eus2_vnet_01_snet_01 = {
        name    = "con-{{configs.environment}}-hub-ntw-00-00-a-snet-iaas1-e2" 
  ...
  cus_vnet_01 = {
    ...
    vnet = {
      name          = "con-{{configs.environment}}-hub-ntw-00-00-a-vnet-00-c1"
      address_space = ["{{configs.connectivity_cus_vnet_01_cidr}}"]      
    }
    specialsubnets = {

      AzureFirewallSubnet = {           # /26 or larger
        name = "AzureFirewallSubnet" #Must be called AzureFirewallSubnet, NSG not supported on Firewall subnet
        cidr = ["{{configs.connectivity_cus_vnet_01_azurefirewallsubnet_cidr}}"] 
      ...
      GatewaySubnet = {         # /29 or larger, /26 or larger recommended
        ...
        cidr = ["{{configs.connectivity_cus_vnet_01_gatewaysubnet_cidr}}"]
      }
    ...
    subnets = {
      AzureBastionSubnet = {            # /27 or larger
        name    = "AzureBastionSubnet" #Must be called AzureBastionSubnet
        cidr    = ["{{configs.connectivity_cus_vnet_01_azurebastionsubnet_cidr}}"]
        nsg_key = "cus_azure_bastion_nsg"
      }
    

      cus_vnet_01_snet_01 = {
        name    = "con-{{configs.environment}}-hub-ntw-00-00-a-snet-iaas1-c1" 
        cidr    = ["{{configs.connectivity_cus_vnet_01_snet_01_cidr}}"]
        nsg_key = "cus_nsg_default"
        route_table_key = "cus_rt_01"
